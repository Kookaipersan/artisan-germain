...
import "./BatimentPage.scss";
import { Link } from "react-router-dom";


function BatimentPage() {
  ...
  useEffect(() => {
   fetch("https://artisan-germain-backend.onrender.com/api/artisans") 
      ...
      .then((data) => {
        ...
        console.log("Liste complète des artisans :", data);
        const filtered = data.filter((artisan) =>
          [5, 6, 7, 8].includes(artisan.specialite_id) // ← Ajuste cette liste aux ID des spécialités bâtiment
        );
  ...
  return (
    <section className="batiment-section py-5">
      <div className="text-center mb-4">
        <h2 className="batiment-title">Artisans du Bâtiment</h2>
